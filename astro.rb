=begin
astro.rb - astronomical (Ruby) calculations
...
=== Content
Library of functions related with astronomy which implement verifiable
calculations according to known and verified astronomical sources.
...
# (test indicate 9 decimal digits precision, so 8 will be on the safe side)
DECIMAL_DIGITS = 8

# true if date correspond to a julian date, false if the date correspond to
# the gregorian calendar
#
# === Example
#   test_juliandate(2000,1,31)    # <= false
def test_juliandate(year, month, day)
  (year < GREGORIAN_CHANGE[0] or
    (year == GREGORIAN_CHANGE[0] and
      (month < GREGORIAN_CHANGE[1] or
        (month == GREGORIAN_CHANGE[1] and
          day < GREGORIAN_CHANGE[2]))))
end
...
It will raise an error if you try to calculate negative values for julian
day (i.e. before year -4712)

=== Note
If you are interested only in julian day calculation maybe you can skip
using this library at all as apparently you can get the same results with
either:
...
def calculate_jd(year, month, day, force_juliandate = false)
  fail(ArgumentError,
       'julian day before year -4712 not supported') if year < -4712
  fail(ArgumentError,
       "year or month can't be fractional") if year % 1 != 0 or month % 1 != 0
  fail(ArgumentError,
       'month 1-12, day 1-31') unless month > 0 and month <= 12 and day > 0 and day <= 31
  ...
  if test_juliandate(year, month, day) or force_juliandate
  ...
  partial = (365.25 * (year + 4716)).to_i + (30.60001 * (month + 1)).to_i + b
  # partial is an int!!
...
def calculate_yearday(year, month, day)
  ...
  (calculate_jd(year, month, day) -
   calculate_jd(year, 1, 1) +
   1).round(DECIMAL_DIGITS)
end
...
def calculate_ymd(julian_day)
  ...
  fail(ArgumentError,
       'negative julian day not supported') if julian_day < 0
...
class DateAstro
  ...
  # === Example
  #   astro = DateAstro.new(2000,1,31)
  #   astro.juliandate?    # <= false
  def juliandate?
    Astro.test_juliandate(@year, @month, @day)
  end

  # returns Modified Julian Day (MJD)
  def mjd
    Astro.calculate_mjd(jd)
  end

  # prints a complete report on the date
  def report
    printf("year/month/day      #{@year}/#{@month}/#{day} (%s)\n",
           juliandate? ? 'julian' : 'gregorian')
